const checkTimerProreties = (timer, timersArray) => {
  ...
  if (name === undefined || typeof name !== 'string' || name.length === 0) {
    throw new Error('The \'name\' parameter is required and should be a string (string.lenght > 0)');
  }
  ...
  if (delay === 0 || delay > 5000) {
    throw new Error('The \'delay\' parameter should be more then 0 and less then 5000');
  }

  /* interval */
  if (interval === undefined || typeof interval !== 'boolean') {
    throw new Error('The \'interval\' parameter is required and should be a boolean');
  }
  ...
  if (duplicatedTimer) {
    throw new Error(`The timer with the name: '${duplicatedTimer.name}' has already been added.`);
  }
...
class TimersManager {
  ...
  remove(timerName) {
    ...
    try {
      ...
      console.error(`Timer you are trying to remove: '${timerName}' does not exist`);
    }
  }
  ...
  start() {
    ...
    this.timers.forEach(timer => {
      let timerID;
      this.delaySum = this.delaySum + timer.delay
      let promise = new Promise((resolve, reject) => {
        timerID = setTimeout(function tick() {
          resolve(timer.job(timer.arg1, timer.arg2));
      ...
      promise.then(res => this._log(timer, res));
  ...
  pause(timerName) {
    ...
    try {
      ...
      console.error(`Timer you are trying to pause: '${timerName}' does not exist`);
    }
  }
  ...
  resume(timerName) {
    ...
    try {
      ...
      console.error(`Timer you are trying to resume: '${timerName}' does not exist or has been deleted`);
    }
